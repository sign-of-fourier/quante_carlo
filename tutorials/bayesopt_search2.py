...
class BatchBayes:
    ...
    def register(self, p, value_trial_pair):
        self.history.append(value_trial_pair)
...
#        batch_size: int = 1,
        ...
        self.test_optimizer = qc.bayesian_optimization_api(**self.bo_args)
    ...
    def suggest(self, trial_id: str) -> Optional[Dict]:
        ...
        if self._points_to_evaluate:
            ...
            if self.test_optimizer.use_qc:
                config = self.test_optimizer.suggest()
            else:
                config = self.optimizer.suggest()
        with open('debug.txt', 'a') as f:
            f.write("new config :" + str(config)+"\n")
            f.write("history :" + str(self.test_optimizer.historical_parameters)+"\n")
    ...
    def _register_result(self, params: Tuple[str], result: Dict):
        ...
        self.test_optimizer.register(params, self._metric_op * result[self.metric])
